...
export const dashboardService = {
   ...
   */
  async getExpensesByCategoryTimeseries(nMonths?: string): Promise<unknown> {
    const months = Number.parseInt(nMonths || '6', 10);
    const fechaInicio = getFirstDayOfMonthAgo(months);

    const query = `
      SELECT
        categorias.descripcion as categoria,
        SUBSTRING(fecha, 1, 7) as month,
        SUM(monto) as total
      FROM documentos
      INNER JOIN categorias ON documentos.fk_categoria = categorias.id
      WHERE documentos.fk_tipoDoc = 1
      AND documentos.fecha >= '${fechaInicio}'
      GROUP BY categorias.descripcion, SUBSTRING(fecha, 1, 7)
      ORDER BY categorias.descripcion, month
    `;
